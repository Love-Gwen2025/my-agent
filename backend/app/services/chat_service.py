...
class ChatService:
    ...
    async def chat(
        ...
        if self.cache_service:
            ...
            await self.cache_service.add_message(conversation_id, "assistant", reply, assistant_message.id)
        
        if self.embedding_service and db:
            # 异步存储 embedding，不阻塞响应
            asyncio.create_task(self._store_embeddings_async(
                db, user_message.id, assistant_message.id,
                conversation_id, user_id, content, reply
            ))
    ...
    async def stream(
        ...
        """
        # 使用同步方法获取完整回复，然后拆分为流
        reply_full, message_id = await self.chat(user_id, conversation_id, content, model_code, db)
        
        # 模拟流式输出
        tokens = [reply_full[i : i + 10] for i in range(0, len(reply_full), 10)]
        for token in tokens:
            yield json.dumps(
                {
                    "type": "chunk",
