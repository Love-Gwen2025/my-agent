...
-- Redis: 单机实例，用于存储 JWT token（key: agent:token:${token}，值为用户 ID，TTL = jwt_expire_minutes）。
-- MongoDB: 单机实例，用于存储 AI 对话缓存，集合 conversation_cache（字段：conversationId, userId, messages[], updatedAt）。
